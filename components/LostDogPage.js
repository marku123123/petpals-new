...
const LostDogPage = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const lostDogsResponse = await axios.get(
          LOST_DOG_API_URL, // Updated to use constant (NEW)
        ...
        const postsResponse = await axios.get(
          NEW_POSTS_API_URL // Updated to use constant (NEW)
        );
    ...
    const socket = io(SOCKET_URL, { transports: ["websocket"] }); // Updated to use constant (NEW)
  ...
  const handleMoreInfoClick = (dog) => onNavigateToLostDogPageMoreInfo?.(dog);
          ...
          filteredDogs.map((dog, index) => (
            ...
            >
              <Image
                source={
                  dog.imagePath
                    ? { uri: `${BASE_URL}${dog.imagePath}` } // Updated to use constant (NEW)
